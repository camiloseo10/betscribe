...
import { findUserByEmail, createSession, latestVerification, markVerified, completeRegistration } from "@/lib/auth"
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!user) {
      // Si el usuario no existe, intentamos completar el registro con los datos pendientes
      user = await completeRegistration(email)
      if (!user) return NextResponse.json({ error: "No se encontraron datos de registro. Por favor regístrate nuevamente." }, { status: 404 })
    }
