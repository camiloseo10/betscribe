    ...
    try {
        ...
        (0, estree_walker_1.walk)(ast, {
            enter(node) {
                ...
                let betscribeId = `${rel}:${line}:${column}`;
                // Enhance the ID with context if we have map information
                if (mapContext) {
                    betscribeId += `@${mapContext.arrayName}`;
                }
                ...
                ms.appendLeft(node.name.end, ` data-betscribe-id="${betscribeId}" data-betscribe-name="${semanticName}"`);
                mutated = true;
            },
        });
        if (!mutated)
