...
export const CHANNEL = "BETSCRIBE_HOVER_v1" as const;
const VISUAL_EDIT_MODE_KEY = "betscribe_visual_edit_mode" as const;
const FOCUSED_ELEMENT_KEY = "betscribe_focused_element" as const;

// Deduplicate helper for high-frequency traffic (HIT / FOCUS_MOVED / SCROLL)
// -----------------------------------------------------------------------------
let _betscribeLastMsg = "";
const postMessageDedup = (data: any) => {
  try {
    const key = JSON.stringify(data);
    if (key === _betscribeLastMsg) return; // identical – drop
    _betscribeLastMsg = key;
  } catch {
...
// Helper to parse data-betscribe-id to extract file path, line, and column
const parseBetscribeId = (
  betscribeId: string
): { filePath: string; line: number; column: number } | null => {
  // Format: "filepath:line:column"
  const parts = betscribeId.split(":");
...
export default function HoverReceiver() {
  ...
  const protectChildElements = (element: HTMLElement) => {
    ...
    childElements.forEach((child) => {
      ...
      childEl.setAttribute("data-betscribe-protected", "true");
  ...
  const restoreChildElements = (element: HTMLElement) => {
    ...
    protectedElements.forEach((child) => {
      ...
      childEl.removeAttribute("data-betscribe-protected");
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    // Get the element identifier to ensure we're working with the right one
    const betscribeId = element.getAttribute("data-betscribe-id");
    if (!betscribeId) return;
    ...
    if (newText !== oldText) {
      const parsed = parseBetscribeId(betscribeId);
      if (!parsed) return;
  ...
  const handleStyleChange = (
    ...
    // Find ALL elements with the same identifier
    const allMatchingElements = document.querySelectorAll(
      `[data-betscribe-id="${betscribeId}"]`
    ...
    const existingStyles = appliedStylesRef.current.get(betscribeId) || {};
    appliedStylesRef.current.set(betscribeId, { ...existingStyles, ...styles });
  ...
  const handleStyleBlur = (element: HTMLElement) => {
    ...
    const appliedStyles = appliedStylesRef.current.get(betscribeId);
  ...
  const flushImageSrcChange = () => {
    ...
    const betscribeId = imgElement.getAttribute("data-betscribe-id");
    if (!betscribeId) return;
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      if (e.data?.type === "BETSCRIBE_STYLE_UPDATE") {
        const { elementId, styles } = e.data;

        // Find ALL elements with the same identifier
        const allMatchingElements = document.querySelectorAll(
          `[data-betscribe-id="${elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      } else if (e.data?.type === "BETSCRIBE_IMAGE_UPDATE") {
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        // Extract file info from data-betscribe-id
        const betscribeId = element.getAttribute("data-betscribe-id");
        if (betscribeId) {
          const parsed = parseBetscribeId(betscribeId);
          if (parsed) {
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      const hit =
        document
          ...
          ?.closest<HTMLElement>("[data-betscribe-id]") ?? null;
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const hitId = hit.getAttribute("data-betscribe-id");
        ...
        const tagName =
          hit.getAttribute("data-betscribe-name") || hit.tagName.toLowerCase();

        // Update hover boxes immediately for instant feedback
        // Find ALL elements with the same identifier
        const allMatchingElements = document.querySelectorAll(
          `[data-betscribe-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        allMatchingElements.forEach((element) => {
          ...
          const elementId = element.getAttribute("data-betscribe-id");
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      const target = e.target as HTMLElement;
      const hit = target.closest<HTMLElement>("[data-betscribe-id]");
      ...
      if (hit) {
        ...
        // Find ALL other elements with the same identifier and show hover boxes
        const allMatchingElements = document.querySelectorAll(
          `[data-betscribe-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        // Clicked on empty space or element without data-betscribe-id
    ...
    function onMsg(e: MessageEvent<ParentToChild>) {
      ...
      if (e.data.msg === "CLEAR_INLINE_STYLES" && "elementId" in e.data) {
        // Find ALL elements with the same identifier
        const allMatchingElements = document.querySelectorAll(
          `[data-betscribe-id="${e.data.elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      if (e.data.msg === "SHOW_ELEMENT_HOVER" && "elementId" in e.data) {
        ...
        if (allMatchingElements.length > 0) {
          ...
          allMatchingElements.forEach((element) => {
            ...
            if (!tagName) {
              tagName =
                element.getAttribute("data-betscribe-name") ||
